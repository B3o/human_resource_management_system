...
public class AdminController {
    ...
    public String list(Admin admin, HttpServletResponse response) throws Exception {
        ...
        // 判断查询条件是否为空，如果不为空，对条件做数据库模糊查询的处理
     ...
     */
    @RequestMapping("/save")
    public String save(Admin admin, HttpServletResponse response, HttpSession session) throws Exception{
        int resultTotal = 0;
        //如果id为空，则添加管理员，否则修改管理员
        if (admin.getId() == null){
    ...
    public String delete(@RequestParam(value = "ids") String ids,
        ...
        for (int i = 0; i < idsStr.length; ++i){
            //不能删除超级管理员(superadmin) 和 当前登录的管理员
